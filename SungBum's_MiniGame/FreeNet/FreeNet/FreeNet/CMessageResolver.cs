...
namespace FreeNet
{
    class Defines
    {
        public static readonly short HEADRSIZE = 2;
    }
    ...
    internal class CMessageResolver
    {
        ...
        public void on_receive(byte[] buffer, int offset, int transffered, CompletedMessageCallback callback)
        {
            ...
            //원본 버퍼의 포지션 값
            //패킷이 여러개 뭉쳐울 경우 원본 버퍼의 포지션은 계속 앞으로 가야 하는데 그 처리를 위한 변수
            ...
            while(this.remain_bytes > 0)
            {
                ...
                //헤더만큼 못읽은 경우 헤더를 우선으로 읽음
                if(this.current_position < Defines.HEADRSIZE)
                {
                    //목표 지점 설정(헤더 위치까지 도달하도록 설정)
                    this.position_to_read = Defines.HEADRSIZE;

                    completed = read_until(buffer, ref src_position, offset, transffered);
                    if(!completed)
                    {
                        //아직 다 읽지 못했으므로 다음 receive대기
                        return;
                    }

                    //헤더 하나를 온전히 읽어왔으므로 메시지 사이즈 구함
                    this.message_size = get_body_size();

                    //다음 목표 지점(헤더 + 메시지 사이즈)
                    this.position_to_read = this.message_size + Defines.HEADRSIZE;
                }
