...
namespace FreeNet
{
    public class CUserToken
    {
        ...
        public void on_receive(byte[] buffer, int offset, int transgered)
        {
            this.message_resolver.on_receive(buffer, offset, transgered, on_message);
        }
        ...
        public void send(CPacket msg)
        {
            ...
            {
                //큐가 비어 있다면 큐에 추가하고 바로 비동기 전송 메소드 호출
                ...
                //큐에 무언가가 들어 있다면 아직 이전 전송이 완료되지 않은 상태이므로 규케 추가만 하고 리턴한다.
                //현재 수행준이 SendAsync가 완료된 이후에 큐를 검사하여 데이터가 있으면 SendAsync를 호출하여 전송해준다.
        ...
        void start_send()
        {
            lock(this.cs_sending_queue)
            {
                //전송이 아직 완료된 상태가 아니므로 데이터만 가져오고 큐에서 제거 안함
                CPacket msg = this.sending_queue.Peek();

                //헤더에 패킷 사이즈 키록
                msg.record_size();

                //이번에 보낼 패킷 사이즈 만큼 크기를 설정하고
        ...
        public void process_send(SocketAsyncEventArgs e)
        {
            ...
            lock(this.cs_sending_queue)
            {
                ...
                {
                    string error = string.Format("Need to send more! transferred{0}, packet size {1}",
                        e.BytesTransferred, size);
                ...
                lock(cs_count)
                {
                    ++sent_count;
                    {
                        Console.WriteLine(string.Format("process send: {0}, transgerred {1}, sent count{2}",
                ...
                //아직 전송하지 않은 대기중인 패킷이 있다면 다시 전송 요청
                if(this.sending_queue.Count > 0)
                {
                    start_send();
                }    
